...
def build_argparser():
    ...
    args.add_argument("-m", "--model", help="Required. Path to an .xml file with a trained model.",
                      type=str, default="/home/zzb/turtlebots_ws/src/OpenVINO-YOLOV4/frozen_darknet_yolov4_model.xml")
    args.add_argument("-i", "--input", help="Required. Path to an image/video file. (Specify 'cam' to work with "
                                            "camera)", type=str, default="cam")
    ...
    args.add_argument("-loop_input", "--loop_input", help="Optional. Iterate over input infinitely",
                      action='store_true')
    args.add_argument("-no_show", "--no_show",
                      help="Optional. Don't show output", action='store_true')
...
def parse_yolo_region(predictions, resized_image_shape, original_im_shape, params, threshold, is_proportional):
    ...
    objects = list()
    size_normalizer = (resized_image_w, resized_image_h) if params.isYoloV3 else (
...
def get_objects(output, net, new_frame_height_width, source_height_width, prob_threshold, is_proportional):
    ...
    for layer_name, out_blob in output.items():
        ...
        params = [x._get_attributes() for x in function.get_ordered_ops()
...
def main():
    pub = rospy.Publisher('taocmd', String, queue_size=20)
    rospy.init_node('darknet_cam', anonymous=True)
    rospy.Subscriber("shibie", String, callback)
    ...
    if args.labels:
        ...
        labels_map = None
    labels_map = ["redblock", "cube", "blueblock",
                  "xuehua", "redcattle", "tennis", "ADgai", "telunsu"]
    ...
    while (1):
        ...
        global gantama
        s = ""
        if (gantama == 1 or gantama == 2) and ret == True:
            q1 = "0"
            q2 = "0"
            if next_frame_id_to_show in completed_request_results:
                ...
                objects = filter_objects(
                    objects, args.iou_threshold, args.prob_threshold)
                ...
                for obj in objects:
                    ...
                    if obj['ymin'] < 150 and obj['xmax'] > 50 and obj['xmax'] < 580 and gantama == 1:
                        q1 = "2"
                    elif obj['ymin'] > 150 and obj['xmax'] > 50 and obj['xmax'] < 580 and gantama == 1:
                        q2 = "1"
                    if obj['ymin'] > 150 and obj['xmin'] >= 0 and obj['xmin'] < 350 and gantama == 2:
                        q2 = "5"
                        temp = q2+str(350-obj['xmin'])+" "+"%s"
                    elif gantama == 2 and temp == "":
                        q2 = "0"
                        q1 = "0"
                        temp = q2+q1+"%s"
                ...
                if mode_info[mode.current].frames_count != 0:
                    ...
                    mode_info[mode.current].latency_sum += perf_counter() - \
                        start_time
                    ...
                    put_highlighted_text(
                        frame, fps_message, (15, 20), cv2.FONT_HERSHEY_COMPLEX, 0.75, (200, 10, 10), 2)
                    put_highlighted_text(
                        frame, latency_message, (15, 50), cv2.FONT_HERSHEY_COMPLEX, 0.75, (200, 10, 10), 2)
                ...
                print(gantama)
                if gantama == 1:
                    pub = rospy.Publisher('taocmd', String, queue_size=1)
                    temp = ""
                    temp = q1+q2+"%s"
                    pub.publish(temp)
                    print("555555555")
